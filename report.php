...
$url = new moodle_url('/mod/customcertificate/report.php', array('id'=>$id, 'page' => $page, 'perpage' => $perpage));
if ($download) {
...
require_capability('mod/customcertificate:manage', $context);
$customcertificate = new customcertificate($certificate, $context);

// Declare some variables
$strcertificates = get_string('modulenameplural', 'customcertificate');
$strcertificate  = get_string('modulename', 'customcertificate');
$strto = get_string('awardedto', 'customcertificate');
$strdate = get_string('receiveddate', 'customcertificate');
$strgrade = get_string('grade','customcertificate');
$strcode = get_string('code', 'customcertificate');
$strreport= get_string('report', 'customcertificate');
...
if (!$users = customcertificate_get_issues($certificate->id, $DB->sql_fullname(), $groupmode, $cm, $page, $perpage)) {
    echo $OUTPUT->header();
    groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/customcertificate/report.php?id='.$id);
    notify(get_string('nocertificatesissued', 'customcertificate'));
...
if ($download == "ods") {
    ...
    if ($users) {
        foreach ($users as $user) {
            ...
            $myxls->write_string($row, 5, $customcertificate->get_grade($user->id));
...
if ($download == "txt") {
    ...
    if ($users) foreach ($users as $user) {
        ...
        echo $customcertificate->get_grade($user->id). "\t";
        echo $user->code . "\n";
        $row++;
    }
    exit;
...
$usercount = count(customcertificate_get_issues($certificate->id, $DB->sql_fullname(), $groupmode, $cm));
...
foreach ($users as $user) {
    ...
    $date = userdate($user->timecreated) . customcertificate_print_user_files($certificate, $user->id, $context->id);
    $code = $user->code;
    $table->data[] = array ($name, $date, $customcertificate->get_grade($user->id), $code);
}
...
echo $OUTPUT->header();
groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/customcertificate/report.php?id='.$id);
echo $OUTPUT->heading(get_string('modulenameplural', 'customcertificate'));
